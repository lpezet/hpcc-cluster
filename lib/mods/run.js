...
ModClass.prototype._execute_cmd = function( pInstanceName, pIp, pKeyPairFile, pCmd, pPty ) {
	...
	return function() {
		return new Promise( function( resolve, reject ) {
			//const oSsh = require('ssh2-client');
...
ModClass.prototype.run = function( pConfig, pParameters ) {
	...
	return new Promise( function( resolve, reject ) {
		try {/*
			execute_cmd = function( pInstanceName, pIp, pKeyPairFile, pCmd ) {
				return function() {
					return new Promise( function( resolve, reject ) {
						//const oSsh = require('ssh2-client');
						const oHost = "ec2-user@" + pIp;
			...
			*/
			...
			//that.mCloudFormationClient.listStackResources( oListStachResourcesParams, function( error, data ) {
			...
			oGetAllInstances.then( function( pInstanceIds ) {
				...
				that.mCloudClient.describe_instances( oEc2Params ).then( function( data ) {
					try {
						...
						//that.mRoot.handle_error( error3, reject );
						//winston.log('error',error3 );
						reject( error3 );
					}
				}, function( error ) {
					reject( error );
				...
				//that.mRoot_handle_error( pError, reject );
				reject( pError );
			});
		} catch (e) {
