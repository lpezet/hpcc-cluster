...
ModClass.prototype.init = function( pParameters ) {
	...
	if ( Fs.existsSync( oClusterConfigFile ) && ! pParameters.force ) {
		var oErrorMsg = ['File %s already exists. Use argument -f to force initialization (and file will be overwritten).', oClusterConfigFile ];
		console.error( util.format.apply(null, oErrorMsg ) )
		that.mLogger.error(oErrorMsg );
		return Promise.reject( oErrorMsg );
	} else {
