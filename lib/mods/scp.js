...
ModClass = function( pHpccCluster, pLogger, pUtils, pSSHClient ) {
	pHpccCluster.mod( 'scp', this, this.handle );
	...
	return new Promise( function( resolve, reject ) {
		...
		if ( (source.indexOf("@") < 0 && target.indexOf("@") < 0) || ( source.indexOf("@") >= 0 && target.indexOf("@") >= 0 ) ) {
			...
			that.mRoot.get_state().then( function( pState ) {
				that.mLogger.info('source=%s, target=%s', source, target);
				...
				*/
				...
				if ( oTargetHostMatch ) {
					...
					if ( oUsername === '' ) {
						reject(new Error('Must specify username in remote.'));
					}
					
					var oPublicIP = oTargetHost;
					if ( !net.isIP( oTargetHost ) ) {
						oPublicIP = oPublicIPAlias[ '@' + oTargetHost ];
					}
