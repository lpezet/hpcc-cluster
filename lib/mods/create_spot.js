...
const DEFAULT_SETTINGS = {
	...
	TemplateCluster: path.resolve(__dirname, "templates/spot.cluster.template.yaml"),
	TemplateClusterNode: path.resolve(__dirname, "templates/spot.node.template.yaml"),
	WorkDir: path.resolve(process.cwd(), ".hpcc-cluster/")
}

ModClass = function( pHpccCluster, pLogger, pUtils, pCloudClient, pSettings) {
	pHpccCluster.mod( 'create-spot', this, this.handle_create );
...
ModClass.prototype.create_cluster_node_cloudformation_templates = function( pConfig , pParameters ) {
	...
	//console.log("######## MasterTemplateVars #######");
...
ModClass.prototype.create_cluster_cloudformation_template = function( pConfig , pParameters ) {
	...
	*/
	var oSupportFleet = this.mUtils.merge_new_only( pConfig.SupportInstance, {} );
	this.mUtils.merge_new_only( oDefaultInstanceSetup, oSupportFleet );
	oSupportFleet.TemplateURL = this.create_node_template_https_url( pConfig, 'Support' );
	oSupportFleet.NodeId = "";//this.mUtils.pad(i,3);
	oSupportFleet.NodeType = "support";
	oSupportFleet.Capacity = pConfig.Cluster.Supports;
	//oSupportFleet.PrivateIpAddress = this.mUtils.getAddrIP( oAddr );
	oTemplateVars.nodes.push( oSupportFleet );
	...
	*/
	var oSlaveFleet = this.mUtils.merge_new_only( pConfig.SlaveInstance, {} );
	this.mUtils.merge_new_only( oDefaultInstanceSetup, oSlaveFleet );
	oSlaveFleet.TemplateURL = this.create_node_template_https_url( pConfig, 'Slave' );
	oSlaveFleet.NodeId = ""; //this.mUtils.pad(i,3);
	oSlaveFleet.NodeType = "slave";
	oSlaveFleet.Capacity = pConfig.Cluster.Slaves;
	//oSlaveFleet.PrivateIpAddress = this.mUtils.getAddrIP( oAddr );
	oTemplateVars.nodes.push( oSlaveFleet );
	...
	return new Promise( function( resolve, reject ) {
		if( pConfig.DryRun ) {
			...
			var create_s3_params = function( pTemplateFilename, pNodeType ) {
				var oBucket = pConfig.AWS.S3Bucket;
				var oKey = that.create_node_template_s3_key( pConfig, pNodeType );
				that.mLogger.info('Uploading template to S3 [template=%s, bucket=%s, key=%s]...', pTemplateFilename, oBucket, oKey);
			...
			var oSupportNodeTemplateUpload = function() { return that.mCloudClient.s3_upload_file( create_s3_params( oSupportTemplateFilename, 'support' ) ) };
			var oMasterNodeTemplateUpload = function() { return that.mCloudClient.s3_upload_file( create_s3_params( oMasterTemplateFilename, 'master' ) ) };
			var oSlaveNodeTemplateUpload = function() { return that.mCloudClient.s3_upload_file( create_s3_params( oSlaveTemplateFilename, 'slave' ) ) };
